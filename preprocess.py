...
def norm_data(args):
	...
	if args.dataset == 'VCTK-Corpus':
		path = os.path.join(args.base_dir, args.dataset)
		supported_readers = [e for e in os.listdir(path) if os.path.isdir(os.path.join(path,e))]
		return [os.path.join(path, reader) for reader in supported_readers]
...
def main():
	...
	parser.add_argument('--language', default='en_UK')
	parser.add_argument('--voice', default='female')
	parser.add_argument('--reader', default='elizabeth_klett')
	parser.add_argument('--merge_books', default='False')
	parser.add_argument('--book', default='wives_and_daughters')
